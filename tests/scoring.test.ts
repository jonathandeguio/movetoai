...
} from "../modules/scoring/domain/calculate-score.ts";
import { deriveOpportunityScoreBadge } from "../modules/scoring/domain/derive-score-badge.ts";
import {
  formatScoreValue,
  getScoreDimensionLabel,
  getScorePriorityBadgeLabel,
  getScorePriorityBadgeVariant
} from "../modules/scoring/domain/scoring-display.ts";
...
assert.equal(getScorePriorityBadgeLabel("en", "QUICK_WIN"), "Quick Win");
assert.equal(getScorePriorityBadgeVariant("HIGH_RISK"), "danger");
assert.equal(getScoreDimensionLabel("fr", "businessValue"), "Valeur metier");
assert.equal(formatScoreValue("en", 84.4), "84");
