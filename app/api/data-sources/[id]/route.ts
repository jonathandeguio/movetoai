...
const PatchSchema = z.object({
  ...
  systemName: z.string().max(120).nullable().optional(),
...
export async function GET(_req: Request, { params }: RouteContext) {
  ...
  const item = await prisma.dataSource.findFirst({
    where: { id, workspaceId: workspace?.id, deletedAt: null },
    select: { id: true, name: true, systemName: true, classification: true, description: true, slug: true, updatedAt: true },
  });
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  if (d.systemName !== undefined) upd.systemName = d.systemName;
  if (d.classification !== undefined) upd.classification = d.classification;
...
export async function DELETE(_req: Request, { params }: RouteContext) {
  ...
  await prisma.dataSource.update({ where: { id }, data: { deletedAt: new Date() } });
