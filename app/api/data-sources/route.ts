...
export async function GET(request: Request) {
  ...
  const items = await prisma.dataSource.findMany({
    ...
    select: {
      id: true, name: true, systemName: true, classification: true, description: true, slug: true, updatedAt: true,
...
export async function POST(request: Request) {
  ...
  const item = await prisma.dataSource.create({
    data: {
      ...
      name: d.name,
      slug: `${slugify(d.name)}-${shortId()}`,
      systemName: d.systemName ?? null,
      classification: d.classification ?? null,
      description: d.description ?? null,
    },
    select: { id: true, name: true, slug: true, systemName: true, createdAt: true },
  });
