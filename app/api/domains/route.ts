...
export async function GET(request: Request) {
  ...
  const domains = await prisma.domain.findMany({
    ...
    select: {
      ...
      businessUnitId: true,
      updatedAt: true,
      _count: { select: { capabilities: true, processes: true, opportunities: true } },
    },
  });

  return NextResponse.json({ items: domains, total: domains.length });
}
...
export async function POST(request: Request) {
  ...
  const allowed = ["WORKSPACE_ADMIN", "ENTERPRISE_ARCHITECT", "TRANSFORMATION_MANAGER", "ENTERPRISE_ARCHITECT"];
