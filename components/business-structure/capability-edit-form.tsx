...
export function CapabilityEditForm({ capabilityId, initialName, initialDescription }: CapabilityEditFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-lg space-y-4 rounded-2xl border border-[--border] bg-[--bg-card] p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-[--text-primary]">Modifier la capacité</p>
        <button type="button" onClick={() => setOpen(false)} className="text-[--text-muted] hover:text-[--text-primary]">
          <X className="h-4 w-4" />
        </button>
      </div>
      <div className="space-y-2">
        <Label htmlFor="cap-edit-name">Nom</Label>
        <Input
          id="cap-edit-name"
          value={name}
      ...
      <div className="space-y-2">
        <Label htmlFor="cap-edit-desc">Description</Label>
        <textarea
          id="cap-edit-desc"
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          placeholder="Description de la capacité (optionnel)"
          rows={3}
